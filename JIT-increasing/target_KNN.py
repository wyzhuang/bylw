...
def data_enhancement(memory,test_features,temp_predict_y,test_labels):
    ...
    test_memory = {0: test_features[test_labels.flatten() == 0], 1: test_features[test_labels.flatten() == 1]}
    ...
    if len(dict_fasle_pred[0]) > 0:
        k_0=base if (len(test_memory[1])+len(dict_fasle_pred[1])-len(test_memory[0]))//len(dict_fasle_pred[0])<=0 else (len(test_memory[1])+len(dict_fasle_pred[1])-len(test_memory[0]))//len(dict_fasle_pred[0])+2

        for feature in dict_fasle_pred[0]:
            distance=np.linalg.norm(memory[0] - feature, axis=1)
            index_0=heapq.nlargest(k_0, range(len(distance)), distance.take)
            index_0=index_0[1:]
    ...
    if len(dict_fasle_pred[1])>0:
        k_1 = base if (len(test_memory[0]) + len(dict_fasle_pred[0]) - len(test_memory[1])) // len(dict_fasle_pred[1]) <= 0 else (len(
            test_memory[0]) + len(dict_fasle_pred[0]) - len(test_memory[1])) // len(dict_fasle_pred[1]) + 2

        for feature in dict_fasle_pred[1]:
            distance=np.linalg.norm(memory[1] - feature, axis=1)
            index_1=heapq.nlargest(k_1, range(len(distance)), distance.take)
            index_1=index_1[1:]
...
def denoising(memory,test_features,test_labels):
    ...
    for n in range(class_num):
        ...
        # threshold_1 =  len(dict_buggy_class[n])  *2//3
        # threshold_0 = len(dict_clean_class[n])  *2 //3

        index_1 = heapq.nlargest(threshold_1, range(len(dict_buggy_class[n])), dict_buggy_class[n].take)
        index_0 = heapq.nlargest(threshold_0, range(len(dict_clean_class[n])), dict_clean_class[n].take)
        if n == 0:
...
def train_and_predict(project):
    ...
    for test_project in test_projects:
        for i in range(10):
            ...
            count=0
            label_count=0
            denoising_features, denoising_labels = np.empty([0,14]), np.empty([0,1])
            ...
            for test_data in lst_test_data:
                ...
                temp_predict_y=clf.predict(test_features)
                ...
                print("train samples:",len(new_train_features[new_train_labels.flatten() == 0]), len(new_train_features[new_train_labels.flatten() == 1]))
                print("test samples:",len(test_labels[test_labels.flatten() == 0]), len(test_labels[test_labels.flatten() == 1]))


                enhancement_features, enhancement_labels=data_enhancement(memory,test_features,np.round(temp_predict_y),test_labels)

                denoising_features = np.concatenate([denoising_features,enhancement_features, test_features])
                denoising_labels = np.concatenate([denoising_labels,enhancement_labels, test_labels])
