...
models=["within_head","within_newest","within_base","within_time_gauss"]
# models=["within_time_liner","within_time_exp","within_time_gauss"]
# models=["ACE_10","ACE_20","ACE_30","ACE_40","ACE_50","ACE_60","ACE_70"]
# models=["ace","tra","ACE_50"]
dict_model_name={}
...
dict_methods={
    "within_newest":"IL-newest",
    "within_head":"TRD",
    "within_base":"IL-base",
    "within_time":"IL-guass",
    "within_time_liner":"IL-liner",
    "within_time_exp":"IL-exp",
}
...
for metric in metrics:
    ...
    for model in models:
        ...
        if model==models[-1]:
            break
        ace_data=pd.read_csv(project_path+"/{}.csv".format(models[-1]))
        ...
        for project in projects:
            ...
            try:
                ...
                print("出错")
                print(central_data[metric], compare_data[metric])
                lst_pvalue.append(1)
    ...
    for i in range(1,len(models)):
        lst_title.insert(i,"&{}".format(models[i-1].replace("_",r"\_")))
        lst_title.insert(2*i, "&{} vs.{}".format(models[i - 1].replace("_",r"\_"),models[-1].replace("_",r"\_")))
    lst_title.insert(len(models), "&{}".format(models[-1].replace("_",r"\_")))
