...
project_path=r"./metric_result/RF_all"
...
for metric in metrics:
    ...
    for model in models:
        ...
        model_data["task_back"] = model_data["task"]
        model_data["task"] = model_data["task_back"].map(lambda x: x.split("_")[1])
        lst_mean=[]
        ...
        metric_data[model]=np.array(lst_mean.copy())
        if model==models[-1]:
            break
        ace_data=pd.read_csv(project_path+"/{}.csv".format(models[-1]))
        ace_data["task_back"] = ace_data["task"]
        ace_data["task"] = ace_data["task_back"].map(lambda x: x.split("_")[1])
    ...
    for i in range(1,len(models)):
        ...
        lst_title.insert(2*i, "&{} vs.{}".format(models[i - 1],models[-1]))
    lst_title.insert(len(models), "&{}".format(models[-1]))
