...
# fig, axes = plt.subplots(figsize=(14, 9),nrows=1, ncols=1,constrained_layout=True)
fig, ax= plt.subplots(figsize=(9, 6),nrows=1, ncols=1,constrained_layout=True)
...
for metric in metrics:
    ...
    axes=metric_data.plot.box(title="", ax=ax,showfliers=False, patch_artist=None,return_type='dict')


    base=order[0]
    count=0
    color=['r','palegreen','skyblue','darkorange','orange','black','blue','yellow']
    dict_temp={}
    ...
    for i in range(1,len(order)):
        if dict_rank[order[i]]>dict_rank[base]:
            plt.plot([i+0.5, i+0.5], dict_y[metric], c='b', linestyle='--')
        base=order[i]
